...
sample_info <- coldata_H14
sample_name <- gsub(" ", "", sample_info$sample, fixed = TRUE)
filename <- "2 H14_trt vs untrt"

cond <- sample_info$cell  # coldata_untrt, coldata_trt
cond <- sample_info$lps # coldata_Wt, coldata_H14

# coldata_Wt, coldata_untrt, coldata_trt
ref <- sample_name[1:3]
...
barplot(y$samples$lib.size, names=colnames(y), las=2) #check if any discrepancies btw samples
title("Barplot of library sizes")
...
write.csv(res_edgeR, file=paste("results_eisa/", filename, "/results_edgeR.csv", sep=""), row.names=FALSE)
...
n = 20 # top n genes based on FDR to label on volcano plot
mat <- res_edgeR[,-c(1,3)]
colnames(mat)[2] <- "L2FC"
...
sig <- mat$FDR < 0.05
sum(sig)
mat$cols <- ifelse(sig, ifelse(mat$L2FC > 0, "up", "down"), "FDR > 0.05")

ggplot(mat, aes(x = L2FC, y = -log10(FDR), label = topGenes)) +
  geom_point(aes(colour=cols, size=logCPM)) +
  scale_size_continuous(range = c(0,4)) +
  scale_color_manual(values=c("#497AB3FF","#22222244","#E41A1CFF")) +
...
ggsave(paste("results_eisa/", filename, "/volcano_top20.png", sep=""), width=10, height=5)

ggplot(mat, aes(x = L2FC, y = -log10(FDR))) +
  geom_point(aes(colour=cols, size=logCPM)) +
  scale_size_continuous(range = c(0,4)) +
  scale_color_manual(values=c("#497AB3FF","#22222244","#E41A1CFF")) +
  ggtitle(str_sub(filename, 3,-1)) + 
...
ggsave(paste("results_eisa/", filename, "/volcano.png", sep=""), width=10, height=5)

# Delta I vs. Delta E
eimat <- data.frame(Din = rowMeans(Din)[quantGenes], Dex = rowMeans(Dex)[quantGenes])
...
ggsave(paste("results_eisa/", filename, "/DIvDE_top20.png", sep=""), width=10, height=5)
...
ggsave(paste("results_eisa/", filename, "/DIvDE.png", sep=""), width=10, height=5)
